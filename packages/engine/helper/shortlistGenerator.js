...
function buildStudentPercentiles(students) {
  ...
  sorted.forEach((student, index) => {
    const percentile = 1 - index / (sorted.length - 1);
...
function calculateShortlistWeight(student, company, percentile) {
  let weight = 1;
  ...
  const cgpaMargin = student.cgpa - company.cgpaCutoff;
...
function weightedSampleStudents(
  ...
  while (selected.length < count && available.length > 0) {
    const weightedCandidates = available.map((student) => ({
      value: student,
    ...
    const chosen = weightedPick(rng, weightedCandidates);

    selected.push(chosen);

    const index = available.findIndex((student) => student.id === chosen.id);
...
export function generateShortlists(rng, companies, students) {
  const percentileById = buildStudentPercentiles(students);
  ...
  for (const company of companies) {
    ...
    const selectedStudents = weightedSampleStudents(
      rng,
      eligibleStudents,
      actualShortlistSize,
      company,
    ...
    company.shortlistedStudentIds = selectedStudents.map(
      (student) => student.id,
    );
  }
